...
describe('LineChart', () => {
  ...
  it('displays the correct section labels', () => {
    cy.compareInputs('.dt-label-input', '#container-line-test .x-axis text');
  });
  ...
  it('displays the correct key names', () => {
    cy.compareInputs('.dt-key-input', '#container-line-test .key text');
  });
  ...
  it('displays the correct colours', () => {
    ...
    cy.log('...on the chart');
    cy.get('#container-line-test circle.circles').each(($element, i, $collection) => {
      let iDataLength = $collection.length/iColorLength;
      let iColorIndex = (i - (i%iDataLength))/iDataLength;
      expect(fnRgbTohex($element[0].attributes.fill.value)).to.eq(aColors[iColorIndex]);
    });
    cy.log('...on the key');
    cy.get('#container-line-test rect.key').each(($element, i) => {
  ...
  it('allows colours to be changed', () => {
    ...
    cy.get('#container-line-test circle.circles').each(($element) => {
  ...
  it('displays the correct values', () => {
    ...
    cy.get('#container-line-test circle.circles').each(($bar, i, $collection) => {
      let iDataLength = $collection.length/iColorLength;
      let iGroupCount = Math.floor(i/iDataLength);
      let iItemIndex = ((i - (iGroupCount * iDataLength)) * iColorLength) + iGroupCount;
