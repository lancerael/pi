...
const at = (e, t) => (e.length > t ? `${e.slice(0, t)}[...]` : e),
  ...
  E = (e, t) => new Array(e).fill(null).map(t),
...
function Ot(e) {
  ...
  if (
    (s.boxSizing === 'border-box' &&
      (Math.round(a + o) !== t && (a -= N(s, 'left', 'right') + o),
      Math.round(h + n) !== i && (h -= N(s, 'top', 'bottom') + n)),
    !St(e))
  ) {
  ...
  constructor({
    ...
    ;(this.padding = { l: 45, r: 5, t: 25, b: 85 }),
      ...
      (this.setData = (n, a = 'default', h = 'default', c = !0) => {
        ...
        let d = c ? Number(L(u, (b) => L(b.values))) : 0,
          f = Number(H(u, (b) => H(b.values)))
        ...
        return (
          this.dataSets.set(a, { data: u, minValue: d, maxValue: f }),
          this.draw(),
          this
        )
      }),
      ...
      (this.addAxis = (n = 'default', a = 'default', h = 'default') => {
        ...
        if (c === void 0) throw new Error('No valid config provided for axis.')
        return (
          this.axes.set(
            n,
            new Vt({
              ...
              dimensions: this.dimensions,
              truncate: 10,
              axisLabels: ((l = this.configs.get(h)) == null
                ? void 0
                : l.axisLabels) ?? ['', ''],
              scales: c,
            })
      ...
      (this.updateDimensions = () => {
        const {
          ...
          { l, r: u, t: d, b: f } = this.padding
        ;(this.dimensions.left = n),
          (this.dimensions.width = a),
          (this.dimensions.top = h),
          (this.dimensions.height = c),
          (this.dimensions.innerWidth = a - l - u),
          (this.dimensions.innerHeight = c - d - f),
      ...
      (this.redraw = B(this.draw)),
      (this.resizeWatcher = new Dt(this.redraw)),
      (this.label = t),
      (this.container = this.setContainer(
        typeof i == 'string' ? document.querySelector(i) : i
      )),
      (this.title = S(this.container).append('div').attr('class', 'pic-title')),
      (this.d3Svg = S(this.container)
        .append('svg')
        .attr('class', 'pic-svg')
        .attr('width', '100%')
        .attr('height', '100%')),
    ...
    if (
      ...
      (this.move = B((i) => {
        const {
          ...
          { width: h, height: c } = this.tooltip.getBoundingClientRect(),
          { clientX: l, clientY: u } = i,
          d = o + r - l < h + 10,
          f = a + n - u < c + 10
        ;(this.tooltip.style.left = `${l - r + (d ? -h - 10 : 10)}px`),
          (this.tooltip.style.top = `${u - n + (f ? -c - 10 : 10)}px`)
      })),
      (this.hide = () => {
        this.cleanup(),
...
class Vt {
  ...
  constructor({
    ...
    if (!w(t)) throw new Error('No SVG provided to Axis constructor.')
    ;(this.dimensions = s),
      (this.d3Svg = t),
      (this.tooltip = i),
      (this.truncate = r),
...
class Ft {
  constructor({ scaleType: t = 'band', dataSet: i, dimensions: s }) {
    ...
    ;(this.setData = (o) => {
      ...
      (this.scaleType = t),
      (this.axisScale = (r = C[t]) == null ? void 0 : r.call(C)),
      (this.dimensions = s),
      (this.dataSet = i),
  ...
  render() {
    ...
    a
      ...
      a
        ...
        .attr('class', 'pic-key')
        .attr('fill', ({ color: l }) => l)
        .attr('x', (l, u) => V(this.values, u) - 16)
        .attr('y', 0)
        .attr('width', 12)
        .attr('height', 12),
      (r = o - r / 2),
      a.attr('transform', `translate(${r},${n})`)
  }
}
class jt {
  /**
  ...
  constructor({
    ...
    if (
      ((this.bars = []),
      (this.render = ({ reset: h = !1, transition: c = !1 } = {}) => {
        if (this.scales.x === void 0 || this.scales.y === void 0) return
        const { data: l, minValue: u, maxValue: d } = this.dataSet,
          { values: f } = this.config,
          { innerHeight: m, padding: b } = this.dimensions,
          O = this.scales.x.axisScale,
          W = this.scales.y.axisScale,
          G = O.bandwidth() / f.length,
          Z = c ? this.transitionTime : 0
        h &&
          (this.bars.forEach((x, $) => {
            ;(x = this.chartGroup.selectAll(`rect.pic-bars-${$}`).data([])),
              x.exit().remove(),
              (x = void 0)
          }),
          (this.bars = [])),
          f.forEach(({ rgbColor: x, name: $ }, v) => {
            const tt = G * v
            this.bars[v] === void 0 &&
              ((this.bars[v] = this.chartGroup
                .selectAll(`rect.pic-bars-${v}`)
                .data(l)),
              this.bars[v]
                .enter()
                .append('rect')
                ...
                .on('mouseover', ({ target: p }) => {
                  S(p).attr('fill', ut(x).formatHex())
                })
                .on('mouseout', ({ target: p }) => {
                  this.tooltip.hide(), S(p).attr('fill', x.formatHex())
                })
                .attr('class', `pic-bars pic-bars-${v}`)
                .attr('fill', x.formatHex())
                .attr('y', m + b.t)
                .attr('height', 0))
            let A = this.chartGroup
              .selectAll(`rect.pic-bars-${v}`)
              .data(l)
              .attr('x', (p) => Number(O(p.label)) + tt)
              .attr('width', G)
            typeof A.transition == 'function' &&
              (A = A.transition().ease(ot).duration(Z)),
              A.attr('y', (p) => {
                let g = p.values[v]
                return (g = g < 0 ? Math.abs(g) : 0), W(p.values[v] + g)
              }).attr('height', (p) => {
                const g = u < 0 ? Math.abs(d) : 0,
                  et = Math.abs(p.values[v]) - g
                return m - W(et) + b.t
              })
          })
      }),
