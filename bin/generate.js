...
const generate = async () => {
  try {
    ...
    for (const fileName of ['src', ...files]) {
      ...
      if (fileName !== 'src') {
        const fileContents = await fs.readFile(`${destination}/${fileName}`, 'utf8')
        await fs.writeFile(`${destination}/${fileName}`, fileContents.replace(/_LC_NAME_/g, lcName).replace(/_NAME_/g, name))
      }
    }
